...
class DBConn:
    ...
    def last_insert_id(self):
        ...
        return self.raw.execute("SELECT last_insert_rowid() id").fetchone()["id"]
...
def make_invoice(order):
    ...
    c=db()
    c.execute("""INSERT INTO invoices(invoice_no,order_id,payer,amount_cents,pdf_path,created_at)
                 VALUES(?,?,?,?,?,?)
...
def login():
    if request.method == "POST":
        ...
        if is_mobile_device():
            return render_template("login_mobile_.html", error=error)
...
@login_required(["admin","dispatcher"])
def alerts():
    c=db(); rows=c.execute("SELECT o.id,o.order_no,o.status,o.pickup,o.destination,o.transport_type FROM orders o WHERE o.status IN ('PROBLEM','NEW') OR (o.status='OFFERED' AND o.created_at <= (CURRENT_TIMESTAMP - INTERVAL '5 minutes')) ORDER BY o.id DESC").fetchall(); c.close(); return jsonify(alerts=[dict(r) for r in rows])
