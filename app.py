...
class DBConn:
    """Small SQLite/PostgreSQL compatibility wrapper used by ENTLASS-CONNECT."""
    ...
    def _convert_sql(self, sql):
        ...
        # SQLite integer primary-key syntax -> PostgreSQL identity column.
        sql = sql.replace(
        ...
        # PostgreSQL supports ADD COLUMN IF NOT EXISTS. This makes the
        # existing safe demo migrations idempotent on every deploy.
        ...
        # SQLite boolean aggregate -> PostgreSQL integer aggregate.
        sql = sql.replace(
    ...
    def execute(self, sql, params=()):
        if self.postgres:
            ...
            # The order-creation flow needs the generated order id.
            # PostgreSQL has no last_insert_rowid(), so capture it directly.
            if "INSERT INTO orders(" in sql.upper() and "RETURNING" not in sql.upper():
                cur = self.raw.execute(sql + " RETURNING id", params)
                row = cur.fetchone()
    ...
    def executescript(self, sql):
        if self.postgres:
            ...
            for stmt in statements:
                self.raw.execute(self._convert_sql(stmt))
        else:
...
def db():
    if USE_POSTGRES:
        if psycopg is None:
            raise RuntimeError(
                "psycopg fehlt – bitte psycopg[binary] in requirements.txt eintragen."
            )
    ...
    return DBConn(c, postgres=False)
...
def init_db():
    ...
    # Older databases may not have invoices.order_id.
    #
    # IMPORTANT:
    # This migration is intentionally OUTSIDE c.executescript().
    # PostgreSQL must execute it as SQL, not as part of the CREATE TABLE script.
    try:
    ...
    for name, phone, cap in driver_specs:
        username = "fahrer" + str(
            driver_specs.index((name, phone, cap)) + 1
        )
        ...
        c.execute(
            ...
            (
                username,
                generate_password_hash("fahrer123"),
                "driver",
                name
            )
        )
        ...
        if not existing:
            # Frankfurt-area demo positions; drivers are intentionally
            # close but not exact real positions.
...
def capability_ok(driver_cap, requested):
    return (
        ...
        or (
            ...
            and requested in ("rollstuhl", "sitzend")
        )
        or (
            driver_cap == "sitzend"
...
def tariff_for(order):
    ...
    if t == "sitzend":
        return {
            "code": "510000",
            "label": "Sitzendkrankenfahrt – Grundpauschale",
            "base": 2.40,
            "km_price": 2.35,
            "km": km,
    ...
    if t == "rollstuhl":
        return {
            "code": "R-Demo",
            "label": "Rollstuhltransport – Demo-Tarif",
            "base": 19.00,
            "km_price": 2.20,
            "km": km,
            "basis": "Illustrativer Demo-Wert – Vertrag erforderlich"
        }

    return {
        "code": "L-Demo",
        "label": "Liegendtransport – Demo-Tarif",
        "base": 49.00,
        "km_price": 2.50,
        "km": km,
        "basis": "Illustrativer Demo-Wert – Vertrag erforderlich"
...
def make_invoice(order):
    ...
    gross = round(
        tariff["base"] + tariff["km_price"] * tariff["km"],
        2
    )
    ...
    try:
        ...
        header = Table(
            [[
                ...
                Paragraph(
                    "<b>ENTLASS</b><br/>"
                    "<font color='#2e9a8b'><b>CONNECT</b></font><br/>"
                    "<font size='8'>DIGITAL. SICHER. GEMEINSAM.</font>",
                    styles["Normal"]
                ),
                Paragraph(
                    "<b>KOSTENTRÄGERRECHNUNG</b><br/>"
                    "<font size='9'>Patiententransport · Demo</font>",
                    styles["Normal"]
                )
            ]],
    ...
    story += [
        ...
        Paragraph(
            f"<b>Rechnungsdatum:</b> {datetime.now():%d.%m.%Y}",
            styles["Normal"]
        ),
        Paragraph(
            f"<b>Leistungsdatum:</b> {order['date']}",
            styles["Normal"]
        ),
        Spacer(1, 12)
    ]
    ...
    recipient = Table(
        [
            ...
            [
                Paragraph(
                    "ENTLASS-CONNECT Patiententransport<br/>"
                    "Demo-Fahrdienst<br/>"
                    "IK: DEMO-IK-123456789",
                    styles["Normal"]
                ),
                Paragraph(
                    f"{order['payer'] or 'Krankenkasse'}<br/>"
    ...
    patient = Table(
        [
            ...
            [
                "Fahrt",
                f"{order['date']} · {order['pickup_time']}",
                "Fahrtrichtung",
                (order["direction"] or "hinfahrt")
                .replace("_", " ")
                .title()
            ],
            [
    ...
    story.append(
        Paragraph(
            ...
            "Die konkrete Vergütung ist vor Produktiveinsatz anhand des "
            "jeweiligen Krankenkassen-/Leistungserbringervertrags und "
            "Abrechnungswegs zu hinterlegen.",
            styles["Normal"]
        )
    )
    ...
    story.append(
        Paragraph(
            "Zuzahlung: grundsätzlich 10 % je Fahrt, mindestens 5,00 € "
            "und höchstens 10,00 €, soweit keine Befreiung bzw. "
            "gesetzliche Ausnahme vorliegt.",
            styles["Normal"]
        )
    )
    ...
    story.append(
        Paragraph(
            "<b>DEMO-DOKUMENT</b> – Diese Rechnung dient ausschließlich "
            "der Funktionsdemonstration von ENTLASS-CONNECT und ist keine "
            "echte Abrechnung mit einer Krankenkasse.",
            styles["Normal"]
        )
    )
    ...
    c.execute(
        "UPDATE orders SET invoice_no=?,price_cents=?,copay_cents=? WHERE id=?",
        (
            inv,
            round(gross * 100),
            round(copay * 100),
...
def dashboard():
    ...
    orders = c.execute(
        """SELECT o.*, o.id AS order_id, d.name AS driver_name
           FROM orders o
    ...
    drivers = c.execute(
        "SELECT * FROM drivers ORDER BY capability,name"
    ).fetchall()

    alerts = c.execute(
        "SELECT * FROM orders "
        "WHERE status IN ('PROBLEM') OR status='NEW' "
    ...
    stats = {
        "today": c.execute(
            "SELECT COUNT(*) n FROM orders WHERE date=?",
        ...
        "available": c.execute(
            "SELECT COUNT(*) n FROM drivers WHERE available=1"
        ).fetchone()["n"],

        "completed": c.execute(
            "SELECT COUNT(*) n FROM orders WHERE status='COMPLETED'"
        ).fetchone()["n"],
    }
    ...
    if request.method == "POST":
        ...
        c.execute(
            ...
            (
                ...
                token,
                0,
                data.get("direction", "hinfahrt"),
                data.get("treatment_facility", ""),
                float(data.get("distance_km") or 12.4),
                0
            )
        )

        oid = c.last_insert_id()

        c.execute(
            "INSERT INTO events(order_id,status,note,created_at) "
            "VALUES(?,?,?,?)",
            (
                oid,
                "NEW",
        ...
        # Automatic demo dispatch: the system selects an eligible
        # available driver.
        candidates = c.execute(
            "SELECT * FROM drivers "
            "WHERE available=1 AND shift_active=1"
        ).fetchall()
        ...
        if eligible:
            d = max(
                eligible,
                key=lambda x: driver_score(x, order_row)[0]
            )

            c.execute(
                "UPDATE orders "
                "SET driver_id=?,status='OFFERED' "
                "WHERE id=?",
                (
                    d["id"],
                    oid
            ...
            c.execute(
                ...
                (
                    oid,
                    "OFFERED",
                    f"Automatisches Angebot an {d['name']} – 5 Minuten",
                    datetime.utcnow().isoformat()
                )
            )
            ...
            c.execute(
                ...
                (
                    oid,
                    "ALARM",
                    "Kein passender Fahrer verfügbar – "
                    "Dispatcher/Admin muss eingreifen.",
                    datetime.utcnow().isoformat()
                )
            )
...
def driver_score(d, order):
    ...
    dist_score = max(
        0.0,
        100.0 - min(dist, 20.0) * 5.0
    )

    capability_score = (
        100.0
        if d["capability"] == order["transport_type"]
        else (
            92.0
            if d["capability"] in ("liege", "rollstuhl")
            else 85.0
        )
    )
    ...
    availability_score = (
        100.0
        if d["available"] and d["shift_active"]
        else 0.0
    )
...
def api_dispatch(order_id):
    ...
    requested_driver_id = (
        requested_driver_id.get("driver_id")
    ...
    if requested_driver_id:
        ...
        if (
            ...
            return jsonify(
                error="Dieser Fahrer ist nicht verfügbar "
                      "oder nicht für die Transportart geeignet."
            ), 409
        ...
        if not eligible:
            ...
            return jsonify(
                error="Kein passender Fahrer verfügbar. "
                      "Dispatcher/Admin wird informiert.",
                alarm=True
            ), 409

        d = max(
            eligible,
            key=lambda x: driver_score(x, o)[0]
        )

    now = datetime.utcnow().isoformat()

    c.execute(
        "UPDATE orders SET driver_id=?,status='OFFERED' WHERE id=?",
        (
            d["id"],
            order_id
        )
    ...
    c.execute(
        "UPDATE drivers SET last_offer_at=? WHERE id=?",
        (
            now,
            d["id"]
        )
...
def fahrer_mobile():
    # Mobile/iOS compatibility endpoint.
    # Keeps the existing driver workflow unchanged while providing
    # the intended mobile URL.
    return redirect(url_for("driver"))
...
def driver_availability():
    ...
    if val:
        ...
        c.execute(
            ...
            "driver_id,started_at,created_at"
            ") VALUES(?,?,?)",
            (
                d["id"],
                now,
...
def driver_permissions():
    # Safari/iOS can revisit this URL with GET.
    # GET must not change permissions; it simply returns the driver
    # to the mobile UI.
...
def driver_problem(order_id):
    ...
    c.execute(
        "UPDATE orders SET status='PROBLEM' WHERE id=?",
        (order_id,)
    )
...
def driver_accept(order_id):
    ...
    c.execute(
        "UPDATE drivers SET available=0 WHERE id=?",
        (d["id"],)
    )
...
def driver_status(order_id):
    ...
    c.execute(
        ...
        (
            ns,
            now if ns == "COMPLETED" else o["completed_at"],
            order_id
        )
    )
